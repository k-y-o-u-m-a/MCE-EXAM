...
//$strQuery = "Select um.userid,um.SName,um.Address,um.exam_center_address,um.lock_login,us.time_left from user_master um,user_session us where md5(um.userid)=us.session_id && um.online='Y'";
$strQuery = "SELECT `exam_date`,`batch`,count(*) FROM `user_master` WHERE `lock_login`='Y' Group By `exam_date`,`batch` Order By `exam_date`,`batch` ASC";
...
//{
    ...
    while ($row = mysqli_fetch_array($result1))
	{
        array_push($arrData["data"], array(
            "label" => $row["batch"].' - '.$row["exam_date"],
            "value" => $row["count(*)"],
            "link"  => "newchart-json-" . $row["batch"]
        ));
